...
function createPointCloud(positions, colors=null, pointSize=2.5){
    ...
    const newPoints = new THREE.Points(pointsGeometry, material)
...
function initUI(zeta=2, n=1, l=0){
    const gui = new dat.GUI({title: 'Quantum Numbers', width: 350})
    ...
    const verifyUserInput = (zeta, n, l) => {
        if(n <= l){
            console.log('invalid')
            points = scene.getObjectByName(points.name)
    ...
    gui.add(userInput, 'zeta', 1, 10, 0.1).name('Atomic Charge, or zeta').onChange(zeta => {
            verifyUserInput(zeta, userInput.n, userInput.l)})
    gui.add(userInput, 'n', 1, nMax, 1).name('Principal, or n').onChange(n => {
            verifyUserInput(userInput.zeta, n, userInput.l)})
    gui.add(userInput, 'l', 0, nMax-1,1).name('Orbital, or l').onChange(l => {
